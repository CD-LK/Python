...
def enter_path() -> Tuple[str, bool, str]:
    path = os.getcwd()
    path = path.replace(' ', '_')
    path = path.replace('(', '[')
    path = path.replace(')', ']')
...
def creatacc() -> Tuple[str, str, bool]:
    ...
    while True:
        ...
        if name_chek == True:
            ...
            chek = back()
            if chek == False:
                return acc_name, acc_name, False
            elif chek == True:
                continue
            ...
            while True:
                ...
                if password_1 != password_2:
                    ...
                    chek = back()
                    if chek == False:
                        return acc_name, password_1, False
                    elif chek == True:
                        continue
...
def autacc() -> Tuple[
    str, str, str, str, bool]:  # false - если аккаунта не существует, или если превышен лимит ввода пароля. В противном случаи возвращает имя, пароль и путь
    print("=====Аутентификация аккаунта=====")
    acc_name, name_chek, fullpath = enter_path()
    if name_chek == False:
        print("Аккаунта не существует, создайте аккаунт.\n"
              "=====Возврат в главное меню=====")
        return acc_name, acc_name, acc_name, acc_name, False
    elif name_chek == True:
        flag = 0
        way = os.path.join(fullpath, inf_acc_de)
        way_key = os.path.join(fullpath, inf_key_de)
        while True:
            ...
            if check == True:
                ...
                return fullpath, password, acc_name, way_key, True
            elif check == False:
                if flag == 2:
                    print("=====Превышен лимит неверного ввода=====\n"
                          "=====Возврат в главное меню=====")
...
def create_path_note_1(path_dir: str) -> Tuple[str, bool]:
    flag = True
    count = 0
    while True:
        ...
        if flag == False:
            if count == 3:
                print("=====Превышен лимит неверного ввода=====")
                return note_name, False
            continue
        note_name = note_name + note_format_1
        note_way = os.path.join(path_dir, note_name)
        return note_way, True


def create_path_note_2(path_dir: str) -> Tuple[str, bool]:
    count = 0
    flag = True
    while True:
        note_name = str(input("Введите название заметки.\n"
...
# Dir = List[str]
def list_dir(acc_name: str, dirrection: str) -> None:
