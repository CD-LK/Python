...
def Atkin(
        num: int, load_x: int, load_y: int) -> Tuple[
    bool, int, int]:  # Алгорит Аткина для одного числа, не убирает числа делимые на полный квадрат простых чисел
    count_1, count_2, count_3 = 0, 0, 0
    for x in range(load_x, int(math.sqrt(num)) + 1):
        for y in range(load_y, int(math.sqrt(num)) + 1):
            n = 4 * x ** 2 + y ** 2
    ...
    if count_1 % 2 == 1 or count_2 % 2 == 1 or count_3 % 2 == 1:
        # print(str(num) + ' True')
        return True, save_x, save_y
    else:
        # print((str(num) + ' False'))
        return False, save_x, save_y
    ...
    try:
        for i in range(len(data_list)):
            if i == 0:
                x = 1
                y = 1
            is_prime, new_x, new_y = Atkin(data_list[i], x, y)
            if x != 1 and y != 1:
                x = int(new_x - 1)
                y = int(new_y - 1)
            if is_prime == True:
